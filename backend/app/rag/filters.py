...
class MetadataFilterBuilder:
    def build_filter_levels(self, context: RetrievalContext, target: EvidenceTarget) -> list[FilterCandidate]:
        base = [f'{MilvusField.DOCUMENT_TYPE} == "{target.document_type}"']
        if context.event_type:
            # event_types (array), not event_type, so cross-cutting docs still match.
            base.append(f'ARRAY_CONTAINS({MilvusField.EVENT_TYPES}, "{context.event_type}")')
        ...
        if context.recall_number and target.document_type == "recall_notice":
            strong.append(f'{MilvusField.RECALL_NUMBER} == "{context.recall_number}"')
        elif context.rxnorm_rxcui and target.document_type == "label":
            strong.append(f'{MilvusField.RXNORM_RXCUI} == "{context.rxnorm_rxcui}"')
        elif context.normalized_drug_name and target.document_type in {"label", "recall_notice"}:
            strong.append(f'{MilvusField.NORMALIZED_DRUG_NAME} == "{context.normalized_drug_name}"')
    ...
    def _section_expr(self, sections: list[str]) -> str:
        if len(sections) == 1:
            return f'{MilvusField.SECTION} == "{sections[0]}"'
        return "(" + " or ".join(f'{MilvusField.SECTION} == "{section}"' for section in sections) + ")"
